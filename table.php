<?php
	...
	*/
	if(isset($_GET["delete"])){
		...
		$stmt = $mysql->prepare("UPDATE messages SET deleted=NOW() WHERE id = ?");
	...
	//SQL sentence
	$stmt = $mysql->prepare("SELECT id, recipient, message, created FROM messages WHERE deleted IS NULL ORDER BY created DESC LIMIT 10");
